...
def TResnetLV2(model_params):
    """Constructs a large TResnet model.
    """
    in_chans = 3
    num_classes = model_params['num_classes']
    do_bottleneck_head = False#model_params['args'].do_bottleneck_head
    layers_list = [3, 4, 23, 3]
    model = TResNet(layers=layers_list, num_classes=num_classes, in_chans=in_chans, do_bottleneck_head=do_bottleneck_head, first_two_layers=Bottleneck)
    return model,  model.num_features
